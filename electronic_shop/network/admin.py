...
class NetworkElementAdmin(admin.ModelAdmin):
    list_display = ('name', 'email', 'country', 'city', 'street', 'house_number', 'supplier_link', 'debt', 'created_at')
    list_filter = ('city',)
    actions = [clear_debt]
    ...
    def supplier_link(self, obj):
        if obj.supplier:
            return format_tb('<a href="{}">{}</a>', admin.utils.quote(obj.supplier.pk), obj.supplier.name)
        return '-'
